...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("canvas_sync.log"), logging.StreamHandler(sys.stdout)],
)
...
def setup_args(argv=None):
    ...
    parser.add_argument(
        "-y", "--yes", action="store_true", help="Skip confirmation prompts (assume yes to all)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--list-managed",
        action="store_true",
        help="List all managed courses in the local database",
    )
    parser.add_argument(
        "--delete",
        dest="delete_target",
    ...
    parser.add_argument(
        "--delete-all", action="store_true", help="Delete all managed courses from local DB"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
        help="Launch the interactive main menu (default if no other args provided)",
    )
...
async def sync_courses(canvas_client, state_manager, notebook_client, args):
    ...
    if args.sync_managed_courses:
        ...
        if args.yes:
            ...
            confirm = (
                input(f"Sync all {len(managed_courses)} managed courses? [Y/n]: ").strip().lower()
            )
            if confirm == "n":
    ...
    for course in courses_to_process:
        try:
            ...
            if not nb_id:
                ...
                if not args.yes:
                    choice = (
                        input(f"Notebook for '{course_name}' does not exist. Create it? [Y/n]: ")
                        .strip()
                        .lower()
                    )
                    if choice == "n":
    ...
    try:
        ...
        for i in indices:
            if 0 <= i < len(managed):
                ...
                confirm = input(
                    f"Are you sure you want to delete '{name}' from local DB? (This does not delete the NotebookLM notebook) [y/N]: "
                ).lower()
                if confirm == "y":
                    if state_manager.delete_course(cid):
...
async def async_main(argv=None):
    ...
    has_direct_utility_action = (
        args.list_managed_courses or bool(args.delete_target) or args.delete_all
    )
    # Backward compatibility: `-y` alone still means "run sync non-interactively".
    wants_headless_sync_all = (
        args.yes
        and not has_direct_utility_action
        and not args.interactive
        and not args.sync_managed_courses
    )
